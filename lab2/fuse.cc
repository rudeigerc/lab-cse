...
// response back to fuse with fuse_reply_buf or fuse_reply_err.
//
void fuseserver_write(fuse_req_t req, fuse_ino_t ino, const char *buf, size_t size, off_t off, struct fuse_file_info *fi)
{
#if 1
    // Change the above line to "#if 1", and your code goes here
    int r;
...
//
yfs_client::status fuseserver_createhelper(fuse_ino_t parent, const char *name, mode_t mode, struct fuse_entry_param *e, int type)
{
    int ret;
...
void fuseserver_create(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode, struct fuse_file_info *fi)
{
...
void fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name) {
    ...
    yfs_client::status ret;
    if ((ret = yfs->symlink(link, parent, name, ino_out)) == yfs_client::OK) {
        e.ino = ino_out;

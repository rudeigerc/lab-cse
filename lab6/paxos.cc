         ...
         std::string &v)
{
    ...
    paxos_protocol::preparearg preparearg = { instance, my_n };
    ...
    for (auto &node : nodes) {
        ...
        int ret = conn->call(paxos_protocol::preparereq, me, preparearg, prepareres, rpcc::to(1000));
...
{
    // You fill this in for Lab 6
    paxos_protocol::acceptarg acceptarg = { instance, my_n, v };
    bool r;
    for (auto &node : nodes) {
        handle h(node);
        ...
        ret = conn->call(paxos_protocol::acceptreq, me, acceptarg, r, rpcc::to(1000));
        if (ret == paxos_protocol::OK) accepts.push_back(node);
    }
}
...
{
    // You fill this in for Lab 6
    paxos_protocol::decidearg decidearg = { instance, v };
    int r;
    for (auto &node : accepts) {
        handle h(node);
        rpcc *conn = h.safebind();
        if (conn) conn->call(paxos_protocol::decidereq, me, decidearg, r, rpcc::to(1000));
    }
}
